...
> **本文件里的相对路径都以 `dietcam/` 为根**（本仓库是多项目仓库，DietCam 是其中一个子目录）。
> 从仓库根目录执行时，请把 `tools/xxx` 读作 `dietcam/tools/xxx`；先 `cd dietcam` 则原文照抄即可。
...
| `../.github/workflows/build-apk.yml` | GitHub Actions：自动编译 APK 并上传产物（放在仓库根，GitHub 只认那里） |
